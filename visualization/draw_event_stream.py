...
def plot_events(xs, ys, ts, ps, save_path=None, num_compress=0, num_show=-1,
        size=1, elev=0, azim=45, imgs=[], img_ts=[], show_events=True, show_frames=True, show_plot=False):
...
def plot_events_between_frames(xs, ys, ts, ps, frames, frame_event_idx, save_dir, num_show=1000,
        skip_frames=5, show_skipped=True, elev=0, azim=0, show_events=True, show_frames=True):
    prev_idx = 0
    for i in range(0, len(frames), skip_frames):
        ...
        plot_events(xs[s:e], ys[s:e], ts[s:e], ps[s:e], save_path=fname, num_show=num_show,
                ...
                elev=elev, show_frames=show_frames)
...
if __name__ == "__main__":
    ...
    plot_events_between_frames(xs, ys, ts, ps, frames, frame_idx, args.output_path, num_show=args.num_show,
            ...
            show_events=not args.hide_events, show_frames=not args.hide_frames)
